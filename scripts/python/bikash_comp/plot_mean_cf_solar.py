...
file_suffix = "mean_solar_cf_1979_2020"
...
levels = [0,5,10,15,20,25,30,35,40,50]
cmap_name = "YlOrRd"
...
for s in seasons:
    file_path = os.path.join(data_dir, f"{file_suffix}_{s}.nc")
    if os.path.exists(file_path):
        ds = xr.open_dataset(file_path, decode_times=False)
        data_slice = ds["capacity_factor"].isel(time=0) * 100
...
if seasonal_datasets:
    ...
    cbar.set_label(f"Capacity Factor (%)", fontsize=14, fontweight="bold")
    
    plt.subplots_adjust(wspace=0.15, hspace=0.15)
    
    seasonal_out = f"figures/seasonal_solar_cf_map.png"
    plt.savefig(seasonal_out, bbox_inches="tight", dpi=300)
    print(f"Saved seasonal plot to: {seasonal_out}")
    plt.close()

#ploting one annual plot
annual_file = os.path.join(data_dir, "mean_solar_cf_Annual_1979_2020.nc")

if os.path.exists(annual_file):
    ds_annual = xr.open_dataset(annual_file, decode_times=False)
    annual_data = ds_annual["capacity_factor"].isel(time=0) * 100
    ...
    annual_out = f"figures/annual_solar_cf_map.png"
